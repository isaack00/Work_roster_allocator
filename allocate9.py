def allocate(preferences, officers_per_org, min_shifts, max_shifts):
    ...
    day_shift_graph = [[0 for i in range(len(preferences) + 2 + 30*len(preferences) + 3*30 +len(officers_per_org))] for j in range(len(preferences) + 2 + 30*len(preferences) + 3*30 +len(officers_per_org))]
    ...
    for i in range(30):
        for j in range(len(officers_per_org)):
            ...
            day_shift_graph[len(preferences) + 1 + 30*len(preferences) + 30*3 + j][len(day_shift_graph) - 1] = sum(total_per_shift) * 30
