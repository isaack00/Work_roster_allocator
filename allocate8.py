def allocate(preferences, officers_per_org, min_shifts, max_shifts):
    ...
    day_shift_graph = [[0 for i in range(len(preferences) + 2 + 30*3)] for j in range(len(preferences) + 2 + 30*3)]
    for i in range(30):
        day_shift_graph[0][i*3 + 1] = total_per_shift[0]
        day_shift_graph[0][i*3 + 2] = total_per_shift[1]
        day_shift_graph[0][i*3 + 3] = total_per_shift[2]
        for j in range(len(preferences)):
            if preferences[j][0] == 1:
                day_shift_graph[i*3 + 1][91 + j] =10
            if preferences[j][1] == 1:
                day_shift_graph[i*3 + 2][91 +j] =10
            if preferences[j][2] == 1:
                day_shift_graph[i*3 + 3][91 + j] =10
        
    for i in range(91, len(day_shift_graph) - 1):
        day_shift_graph[i][len(day_shift_graph) - 1] = -1
    for i in range(len(preferences)):
        day_shift_graph[i + 91][len(day_shift_graph) - 1] = graph[1 +i][0]
