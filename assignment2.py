def allocate(preferences, officers_per_org, min_shifts, max_shifts):
    """
		...
		Time complexity analysis : E*Fmax is simply the fulkerson, then n for jobs constasnt times (including the while loop), constant times there is E for workers. there is also a job*worker, where the while loop is. So E*Fmax + contant*E + constant*N + N*E*constant = E*Fmax +N*E. the deep copy is just another one, so it just adds a constant of *2
